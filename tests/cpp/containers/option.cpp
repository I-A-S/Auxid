...
namespace
{
  ...
  {
    [[nodiscard]] auto get_name() const -> const char * override { return "containers::option"; }
    ...
    {
      Option<i32> opt1 = 42;
      if (!check(opt1.is_some(),  "opt1.is_some()"))    return false;
      if (!check_not(opt1.is_none(), "!opt1.is_none()")) return false;
      if (!check_eq(opt1.unwrap(), 42, "opt1.unwrap() == 42")) return false;

      Option<i32> opt2 = containers::nullopt;
      return check(opt2.is_none(), "opt2.is_none()")
          && check_not(opt2.is_some(), "!opt2.is_some()")
    ...
    auto complex_type() -> bool
    {
      ...
      if (!check(str_opt.has_value(), "str_opt.has_value()")) return false;
      if (!check_eq(str_opt.unwrap(), "Test", "str_opt.unwrap() == \"Test\"")) return false;
    ...
    auto std_interop_some() -> bool
    {
      ...
      if (!check(ours.is_some(), "ours.is_some()"))                return false;
    ...
    auto std_interop_none() -> bool
    {
      ...
      if (!check(ours.is_none(), "ours.is_none()")) return false;
    ...
    auto std_interop_move() -> bool
    {
      ...
      return check(ours.is_some(), "ours.is_some()")
          && check_eq(ours.unwrap(), "hello", "ours.unwrap() == \"hello\"");
    }
  };
