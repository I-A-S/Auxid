...
// Auxid vendor patch (2026-05): added `static inline` to `sprp` and
// `is_prime` so this header can be safely included from multiple TUs
// without triggering duplicate-symbol errors at link time. These helpers
// are only used by `make_secret` which we never call.
